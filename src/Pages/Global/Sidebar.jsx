        ...
        <div className={`bg-dark-purple h-screen p-5 pt-8 ${open ? "w-72" : "w-20"}  relative duration-300`}>
                ...
                <ul className="pt-2">
                    {Menus.map((menu,index) =>(
                       <span key={index}>
                        <Link to={`${menu.to?menu.to:'#'}`}  className={`text-gray-300 text-sm flex items-center gap-x-4 cursor-pointer p-2 hover:bg-slate-400 rounded-md mt-2 ${menu.spacing ? "mt-9" : "mt-2"}`}>
